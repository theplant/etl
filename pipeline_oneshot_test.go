...
// a OneShotPipeline (own queue) syncs exactly the records named by the
// filter, pages through them with the same keyset cursor as the incremental
// mode, destroys its jobs on success without enqueueing any time-window
// successor, and coexists with a normally running incremental Pipeline
...
func TestOneShotPipeline(t *testing.T) {
	...
	oneShot, err := etl.NewOneShotPipeline[*etl.Cursor, OptimizedUserFilter](&etl.OneShotPipelineConfig[*etl.Cursor]{
		Source:      source,
	...
	// Unlike the incremental Pipeline, Start must NOT have seeded any job.
	...
	expectedFilter, err := etl.MarshalOneShotFilter(OptimizedUserFilter{IDs: targetIDs})
	require.NoError(t, err)

	t.Run("targeted sync of 9 ids", func(t *testing.T) {
		require.NoError(t, oneShot.Enqueue(ctx, &etl.Cursor{}, OptimizedUserFilter{IDs: targetIDs}))
	...
	t.Run("incremental chain runs unaffected alongside", func(t *testing.T) {
		...
		require.NoError(t, oneShot.Enqueue(ctx, &etl.Cursor{}, OptimizedUserFilter{IDs: []string{"user01"}}))
		require.Eventually(t, func() bool { return countRows(oneShotQueue) == 0 },
	...
	t.Run("failure retries then expires without successor", func(t *testing.T) {
		...
		failingPipeline, err := etl.NewOneShotPipeline[*etl.Cursor, OptimizedUserFilter](&etl.OneShotPipelineConfig[*etl.Cursor]{
			Source:    failing,
			QueueDB:   pipelineSQLDB,
		...
		failingController, err := failingPipeline.Start(ctx)
		require.NoError(t, err)
		defer func() { _ = failingController.Stop(context.Background()) }()

		require.NoError(t, failingPipeline.Enqueue(ctx, &etl.Cursor{}, OptimizedUserFilter{IDs: []string{"user01"}}))
	...
	t.Run("config and filter validation", func(t *testing.T) {
		// Config validation: the one-shot config has no Interval or circuit
		// breaker fields at all; the common required fields are still checked.
		_, err := etl.NewOneShotPipeline[*etl.Cursor, OptimizedUserFilter](&etl.OneShotPipelineConfig[*etl.Cursor]{
			Source:      syncer,
			QueueDB:     pipelineSQLDB,
